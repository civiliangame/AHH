...
Loads the DAM pipeline (Whisper-Small + classification heads) LAZILY on the
first call that needs scoring, then scores each call's caller audio after the
call ends. Results are persisted to scores/{call_id}.json, folded into the
patient's longitudinal record (interactions.py), and broadcast on the live SSE
stream as a metadata event so the dashboard renders them inline.

The model is optional: if the ~702 MB checkpoint or torch isn't present, scoring
degrades gracefully to a recorded "unavailable" status — the conversational
...
def init_pipeline() -> None:
    """Load the ~702 MB DAM checkpoint once, lazily, on first use.

    Raises if the model can't be loaded (missing checkpoint, torch not
    installed) — analyze_call catches that and degrades to 'unavailable'.
    Idempotent: a no-op once the pipeline is loaded.
    """
    global _pipeline
    if _pipeline is not None:
        return
    if not DAM_CHECKPOINT.exists():
        raise FileNotFoundError(
            f"DAM checkpoint not found at {DAM_CHECKPOINT}. Download it "
            "(see dam/README.md) or set DAM_CHECKPOINT to its path.")
...
async def analyze_call(call_id: str, wav_path: Path, duration_seconds: float,
                       phone: str | None = None) -> None:
    """Background task: score the WAV, persist, fold into the patient's record,
    and broadcast a 'scores' metadata event.

    `phone` (when known) keys the patient whose longitudinal score trend this
    call extends. Persisted scores are written regardless; the patient fold-in
    is skipped only when `phone` is absent.
    """
    payload: dict = {"call_id": call_id, "duration_seconds": round(duration_seconds, 2)}

    def _finish(status: str) -> None:
        """Persist scores/{call_id}.json, broadcast, and fold into the patient."""
        payload["status"] = status
        _write_atomic(call_id, payload)
        if phone:
            try:
                interactions.attach_scores(phone, call_id, payload)
            except Exception:
                log.exception("Could not fold scores into patient %s", phone)
    ...
    try:
        if duration_seconds < DAM_MIN_SECONDS:
            ...
            await events.emit_metadata(call_id, {"kind": "scores", **payload})
            return

        # Lazy-load the model on first use. If it can't load (missing checkpoint
        # or torch), record an 'unavailable' status rather than crashing.
        try:
            await asyncio.to_thread(init_pipeline)
        except Exception as e:
            payload["reason"] = f"DAM model unavailable: {e}"
            _finish("unavailable")
            log.warning("DAM unavailable for %s: %s", call_id, e)
            await events.emit_metadata(call_id, {"kind": "scores", **payload})
            return
